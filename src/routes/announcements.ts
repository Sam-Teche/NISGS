...
router.get('/', async (req, res) => {
  try {
    const announcements = await Announcement.find({ isPublished: true }).sort({ createdAt: -1 }).limit(20);
...
router.post('/', adminMiddleware, async (req, res) => {
  try {
    ...
    if (sendEmail) {
      ...
      if (targetParts && targetParts.length > 0) query.part = { $in: targetParts };
      if (excludedStudents && excludedStudents.length > 0) {
        query._id = { $nin: excludedStudents };
      }
      const students = await Student.find(query).select('email firstName surname');
      const recipients = students.map(s => ({ email: s.email, name: `${s.firstName} ${s.surname}` }));
      if (recipients.length > 0) {
        sendAnnouncementEmail(recipients, title, content)
          .then(async () => {
            await Announcement.findByIdAndUpdate(announcement._id, { emailSent: true });
          })
          .catch(console.error);
      }
    }
...
router.get('/students-for-exclusion', adminMiddleware, async (req, res) => {
  try {
    ...
    const students = await Student.find(query).select('_id matricNumber surname firstName part');
    res.json(students);
  } catch { res.status(500).json({ message: 'Server error' }); }
});
