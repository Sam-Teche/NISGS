...
// Add student
router.post('/', adminMiddleware, imageUpload.single('photo'), async (req, res) => {
  try {
    const { matricNumber, surname, firstName, email, part } = req.body;
    const existing = await Student.findOne({ matricNumber: matricNumber.toUpperCase().trim() });
    if (existing) return res.status(400).json({ message: 'Matric number already registered' });
...
// Update student
router.put('/:id', adminMiddleware, imageUpload.single('photo'), async (req, res) => {
  try {
    const update: any = { ...req.body };
    if (req.file) update.photo = `/uploads/images/${req.file.filename}`;
    if (update.matricNumber) update.matricNumber = update.matricNumber.toUpperCase().trim();
