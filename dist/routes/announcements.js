...
router.get('/', async (req, res) => {
    try {
        const announcements = await Announcement_1.default.find({ isPublished: true }).sort({ createdAt: -1 }).limit(20);
        ...
        if (sendEmail) {
            ...
            if (recipients.length > 0) {
                (0, email_1.sendAnnouncementEmail)(recipients, title, content)
                    .then(async () => {
                    await Announcement_1.default.findByIdAndUpdate(announcement._id, { emailSent: true });
                })
                    .catch(console.error);
            }
        }
