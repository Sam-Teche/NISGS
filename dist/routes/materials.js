...
const buildStoragePath = (courseCode, type, fileName) => {
    ...
    try {
        ...
        if (!type || !courseCode || !courseTitle || !part)
            return res.status(400).json({
                message: "Missing required fields: type, courseCode, courseTitle, part.",
            });
    ...
    try {
        ...
        for (const m of materials) {
            try {
                ...
                results.errors.push(`Error for ${m.courseCode || "unknown"}: ${err.message}`);
                results.skipped++;
            }
        }
        res.status(201).json({
...
router.get("/:id/file", auth_1.studentMiddleware, async (req, res) => {
    try {
        ...
        if (isDownload) {
            Material_1.default.findByIdAndUpdate(req.params.id, {
                $inc: { downloads: 1 },
        ...
        const fileName = safeFileName(`${material.courseCode}_${material.courseTitle}.pdf`);
...
router.patch("/:id", auth_1.adminMiddleware, async (req, res) => {
    try {
        ...
        if (courseTitle !== undefined)
            update.courseTitle = courseTitle.trim();
        if (part !== undefined)
            update.part = Number(part);
        if (year !== undefined)
            update.year = year?.trim() || undefined;
        if (type !== undefined)
            update.type = type;
        if (Object.keys(update).length === 0)
            return res.status(400).json({ message: "No fields provided to update." });
        const material = await Material_1.default.findByIdAndUpdate(req.params.id, update, {
            new: true,
            runValidators: true,
        });
        if (!material)
